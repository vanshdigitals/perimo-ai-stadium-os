...
export const BaseAuthPage: React.FC<BaseAuthPageProps> = ({
  ...
  return (
    <AuthLayout wide={isDemo}>
      ...
      <div className={`bg-white border border-surface-border rounded-2xl shadow-[0_1px_2px_rgba(10,14,20,0.06)] ${
        isDemo ? "overflow-hidden flex flex-col lg:grid lg:grid-cols-2" : "p-[clamp(28px,5vw,40px)_clamp(24px,5vw,36px)]"
      }`}>
        <div className={`order-1 lg:order-none flex flex-col justify-center ${isDemo ? "p-[clamp(32px,5vw,48px)_clamp(28px,5vw,40px)]" : ""}`}>
        ...
        {isDemo && (
          <div className="order-2 lg:order-none bg-surface-subtle border-t lg:border-t-0 lg:border-l border-surface-border p-[clamp(32px,5vw,48px)_clamp(28px,5vw,40px)] flex flex-col justify-center">
            {demoCard}
          </div>
        )}
      </div>
