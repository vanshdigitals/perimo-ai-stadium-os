...
import { LayoutDashboard, UsersRound, FileText, HelpCircle } from 'lucide-react'
...
const NAV_GROUPS = [
  {
    label: '',
    items: [
      { label: 'Volunteer Dashboard', icon: LayoutDashboard, path: '/volunteer' },
      { label: 'Report Incident', icon: FileText, path: '/volunteer/report' },
      { label: 'Wayfinding Map', icon: UsersRound, path: '/volunteer/map' },
    ]
  }
]
...
export const VolunteerSidebar: React.FC<VolunteerSidebarProps> = ({ isOpen, onToggle, isMobileDrawer = false }) => {
  ...
  useEffect(() => {
    ...
    if (!nav) return;
    const savedScroll = sessionStorage.getItem('perimoVolunteerSidebarScroll');
    if (savedScroll) {
      nav.scrollTop = parseInt(savedScroll, 10);
    }
    const handleScroll = () => {
      sessionStorage.setItem('perimoVolunteerSidebarScroll', nav.scrollTop.toString());
    };
    ...
    >
      <div className="flex flex-col h-full bg-white overflow-hidden">
        ...
        <div className="shrink-0 pt-3 pb-4 border-t border-[#E2E8F0] flex flex-col gap-1 bg-white z-10 shadow-[0_-4px_12px_rgba(255,255,255,0.9)]">
          {[
            { label: 'Help', icon: HelpCircle, path: '/volunteer/help' },
          ].map((item) => {
